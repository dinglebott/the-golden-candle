...
2. `dataparser.parseData()` unpacks raw JSON into a DataFrame and engineers ~45 features (returns, ATR, Bollinger, EMAs, ultSmoother variants, RSI, MACD, ADX, Williams %R, time-of-day sin/cos, directional features, and Ehlers EMD bandpass features — `bp`/`avg_peak`/`avg_valley`/`regime`/`bp_norm`/`cycle_strength`, where `regime` flags cyclic vs trending conditions)
...
**Strategies:**
- `macd.py` — MACD signal-line crossover (a `macd_hist` sign change). Long on a bullish cross, short on a bearish cross, gated by three direction-symmetric entry filters: EMD `regime==0` (non-cyclic/trending), a volume surge (`vol_ratio > VOL_RATIO_MIN` and rising `vol_momentum`), and a low-extension filter (`|dist_ema15| <= DIST_EMA15_MAX`, i.e. entry near EMA15). Exits are independent fixed ATR multiples — SL `SL_ATR_MULT × ATR`, TP `TP_ATR_MULT × ATR` (1.0 / 2.0, RR 2.0). Symmetric long/short. This is the reference strategy produced by the tuning workflow below; older strategies (london_orb, trend_pullback, liquidity_sweep, mean_reversion) were removed pending revision under that workflow.

**Tuning workflow:** `TUNING_GUIDE.md` documents the methodology for building/refining a strategy — baseline the bare idea → breadth-first search for filter/confluence *combinations* that work on train → fine-tune the finalists and keep only changes that survive the held-out test split (picking robust regions over train peaks). Per-strategy records live in `claude_tuning_logs/<strategy>.md` (e.g. `macd.md`), logging both what worked and what was tried-and-refuted. `visualise.py` renders `COUNT` random trade snapshots (H1+H4) to `trade_snapshots/<strategy>/` for diagnosing failure modes the metrics miss.

**Key `env.json` fields:**
- `strategy` — module name of the active strategy (e.g. `"macd"`)
...
`BE_TRIGGER_R` at the top of `backtest.py` enables a break-even SL move once price reaches that R-multiple of favourable excursion (set to `0` to disable; currently `0` — it was tested and hurt the macd strategy's high-payoff profile). Strategy-specific knobs (SL/TP multiples, filter thresholds, lookbacks) live as module-level constants at the top of each strategy file.
...
Adding a new strategy endpoint: the generic `/strategy/{name}` handler reads from `STRATEGY_REGISTRY` in `main.py`. Each strategy module in `dist/api/` exposes `get_entries(df, ..., n_active=6) -> dict | None` — a single entry dict `{direction, entry, tp, sl, tp_type?, time}` or `None`, NOT the backtest's per-bar DataFrame. The registry entry pairs the module with a `fetch` callable that returns `(kwargs_for_get_entries, timestamp)`; the current fetcher is `_fetch_h1_only` in `main.py`. A strategy that needs long-lookback features (e.g. daily EMA200) takes them as separate kwargs and a fetcher pulls them via a separate OANDA call (`getData(..., gran="D", dailyAlignment=0, alignmentTimezone="UTC")` → `parseDailyOhlc()`) so the H1 fetch stays small; such a daily+H1 fetcher previously existed for `trend_pullback` and can be re-added when needed. `STRATEGY_VERSIONS` entries in `inference.py` are purely semantic — stamped into the response as the UI version badge, no file paths depend on them.

The streamlined deployment copy of a strategy differs from the backtest source in: (1) `get_entries` returns a dict-or-None instead of a DataFrame, (2) it walks the full df but only retains entries triggered in the last `n_active` H1 bars, (3) `tqdm` and other backtest-only scaffolding is removed. Strategies that resample to a higher timeframe additionally skip arming on a trailing H1 bar whose higher-TF bucket is still incomplete, and those needing long-lookback features accept them as kwargs rather than resampling from the H1 fetch. (The current `macd.py` is pure H1, so neither of those extra cases applies to it.)

**Critical sync points:** `dist/api/data_processing.py` is a copy of `data_processing/dataparser.py` (plus live `getData()` and `parseDailyOhlc()`). `dist/api/symmetry.py`, `dist/api/fair_value_gap.py`, `dist/api/order_block.py`, `dist/api/patchtst.py`, and `dist/api/cnn_lstm.py` are copies of their counterparts in `2_event-detection/` and `1_double-binary/PatchTST/`. `dist/api/macd.py` is a streamlined copy of `3_algo-backtest/macd.py` (see above for the deployment-vs-backtest differences). When source logic changes, copies must be manually synced — note `regime` and the other EMD features the strategy relies on come from `data_processing.py`, so that copy must carry the EMD block too.
