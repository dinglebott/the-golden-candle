...
df = dataparser.parseData(_data_file)
...
# Metadata features only exist at the detection candle; they are injected after the TCN.
...
lines = []
lines.append(f"=== v{version} | {instrument} {granularity} | {pattern} | TCN | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===")
...
model_path = Path(__file__).parent / f"models/{pattern}_TCN_{instrument}_{granularity}_{year_now}_v{version}.pt"
