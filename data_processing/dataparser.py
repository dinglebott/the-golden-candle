...
# 1st binary target: flat(0) or directional(1)
def addGateTarget(df, k, time_horizon):
    raw_atr = df["raw_atr"].to_numpy()
    close = df["close"].to_numpy()
    high = df["high"].to_numpy()
    low = df["low"].to_numpy()
    n = len(df)
    labels = np.full(n, 0, dtype=np.int64)

    for i in range(n - time_horizon):
        barrier = k * raw_atr[i]
        upper = close[i] + barrier
        lower = close[i] - barrier
        label = 0  # default flat (time barrier expiry)
        for j in range(i + 1, i + 1 + time_horizon):
            hit_up = high[j] >= upper
            hit_dn = low[j] <= lower
            if hit_up and hit_dn:
                break # ambiguous same-candle hit → flat
            if hit_up or hit_dn:
                label = 1
                break
        labels[i] = label

    # drop tail rows without a full time-barrier window
    df = df.iloc[:n - time_horizon].copy()
    df["target"] = labels[:n - time_horizon]
    return df


# 2nd binary target: down(0) or up(1)
def addDirectionTarget(df, k, time_horizon):
    raw_atr = df["raw_atr"].to_numpy()
    close = df["close"].to_numpy()
    high = df["high"].to_numpy()
    low = df["low"].to_numpy()
